...
class InformasiKIA extends StatefulWidget {
  const InformasiKIA({super.key});

  @override
  State<InformasiKIA> createState() => _InformasiKIAState();
}

class _InformasiKIAState extends State<InformasiKIA> {
  ...
  Future<void> fetchInformasi() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("http://10.0.2.2:8000/api/informasi_kia"),
        headers: {
  ...
  Widget build(BuildContext context) {
    return BaseLayouts1(
      ...
      child: isLoading
          ...
          : informasi.isEmpty
              ? const Center(
                  child: Text("Tidak ada informasi untuk pengajuan KIA."),
                )
              : ListView(
                  shrinkWrap: true,
